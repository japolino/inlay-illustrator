...
function setup(ctx) {
  ...
  function render() {
    ...
    const sections = tab.root.querySelector(".inlay-sections");
    updateStatus(status);
    ...
    const addSwitch = (parent, key, label, hint = "") => {
      const target = row(parent, label, hint);
      const component = ctx.components.mountSwitch(target, {
        checked: Boolean(config[key]),
        ariaLabel: label,
        onChange: (checked) => patchConfig({ [key]: checked })
      });
      mountedComponents.push(component);
    };
    const addNumber = (parent, key, label, min, max, hint = "") => {
      const target = row(parent, label, hint);
    ...
    const addSelect = (parent, key, label, options, hint = "") => {
      const target = row(parent, label, hint);
    ...
    const addText = (parent, key, label, hint = "") => {
      const target = row(parent, label, hint);
    ...
    const addTextarea = (parent, key, label, hint = "") => {
      const target = row(parent, label, hint);
    ...
    const addActions = (parent, actions) => {
    ...
    parserParameterInput.addEventListener("change", () => {
      try {
        patchConfig({ parserParameters: JSON.parse(parserParameterInput.value || "{}") });
      } catch {
    ...
    addActions(diagnostics, [{ label: "Refresh state", onClick: () => {
