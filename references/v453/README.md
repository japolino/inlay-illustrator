...
`core/entries.json` and `illustration/entries.json` preserve the lorebook entries. Numbered `.txt` files contain each corresponding entry's content. `toggles.txt` preserves the configuration definitions. `illustration/runtime.lua` and `core/runtime.lua` contain the source trigger implementations.
