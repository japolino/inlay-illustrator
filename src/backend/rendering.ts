...
function renderInlayBlock(
  ...
  const quoteHtml = quote.trim()
    ...
    : "";
  return `${MARKER}\n<div class="inlay-illustrator-image" data-inlay-illustrator="true" style="display:flex;flex-direction:column;justify-content:center;align-items:center;margin:10px 0;width:100%;"><span class="inlay-illustrator-frame" style="display:block;width:${boxWidth};max-width:100%;aspect-ratio:${aspect.w}/${aspect.h};"><img alt="${htmlAttr(label)}" data-inlay-illustrator-quote="${htmlAttr(quote)}" data-inlay-illustrator-image-id="${htmlAttr(imageId)}" data-inlay-illustrator-chat-id="${htmlAttr(chatId)}" data-inlay-illustrator-message-id="${htmlAttr(messageId)}" data-inlay-illustrator-swipe-id="${swipeId}" data-inlay-illustrator-image-index="${index}" data-inlay-illustrator-image-url="${htmlAttr(url)}" src="${htmlAttr(url)}" style="display:block;width:100%;height:100%;object-fit:cover;border-radius:8px;cursor:zoom-in;"/></span>${quoteHtml}</div>`;
}
