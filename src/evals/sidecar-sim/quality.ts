...
const COMPOSITION_CONTAMINATION = /\b(?:close-up|medium shot|wide shot|camera|angle|pov|depth of field|blur|lens|lighting|backlight|rim light|streetlight|coat|shirt|jacket|trousers|pants|skirt|dress|uniform|hair|eyes|smiling|smile|angry|furious|afraid|fear|blush)\b/i;
const ATOMIC_END = /[.!?:,;]\s*$/;
...
function expectedField(payload: ParsedPayload, rendered: Map<number, string>, scenario: SidecarScenario, paragraph: number, character: string | undefined, field: string): string {
  ...
  if (field === "action") return searchable([
...
function validateAnima(payload: ParsedPayload, scenario: SidecarScenario, issues: QualityIssue[]): void {
  ...
  cleanArray<Record<string, unknown>>(payload.scenes).forEach((scene, sceneIndex) => {
    ...
    cleanArray<Record<string, unknown>>(scene.shots).forEach((shot, shotIndex) => {
      exactFields(shot, ["paragraph", "perspectiveMode", "camera", "situation", "characters", "sharedComposition", "negative"], `scenes[${sceneIndex}].shots[${shotIndex}]`, issues);
      ...
      cleanArray<Record<string, unknown>>(shot.characters).forEach((character, characterIndex) => {
        ...
        const values = [...atomicValues(composition.position), ...atomicValues(composition.pose), ...actions, ...atomicValues(composition.gaze)];
        values.forEach((value) => {
          ...
          if (COMPOSITION_CONTAMINATION.test(value)) issues.push(issue("schema", "composition_contamination", `P${shot.paragraph} composition contains tag-domain data: ${value}`));
        });
...
export function evaluateQuality(
  ...
  normalizedEntries.forEach((entry) => {
    ...
    if (JSON.stringify(expected) !== JSON.stringify(actual)) issues.push(issue("semantics", "character_set", `P${entry.parserParagraph} expected ${expected.join(", ")}; received ${actual.join(", ")}`));
  });
  if (scenario.config.promptStyle === "anima") validateAnima(payload, scenario, issues);
