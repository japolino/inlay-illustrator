...
export function renderPromptSection({ ui, config, actions, rerender }: SectionContext): void {
  ...
  const presetPositiveTarget = ui.row(section, "Preset positive", "Tags inserted before the generated positive prompt.");
  ...
  const presetNegativeTarget = ui.row(section, "Preset negative", "Tags inserted before the generated shot negatives.");
